...
def get_timesteps(min_timestep: int, max_timestep: int, b_size: int, device: torch.device) -> torch.Tensor:
    if min_timestep < max_timestep:
        ...
        # Filter by min/max timestep range
        valid_timesteps = [t for t in predefined_timesteps if min_timestep <= t < max_timestep]
        if not valid_timesteps:
            valid_timesteps = predefined_timesteps

        # Weighted sampling based on eval-aligned bias
        timestep_weight_bias = os.environ.get("TIMESTEP_WEIGHT_BIAS", "none")
        weight_mult = float(os.environ.get("TIMESTEP_WEIGHT_MULTIPLIER", "3.0"))
        weight_mid = max(1.0, weight_mult * 0.5)  # mid-zone gets half the boost

        if timestep_weight_bias == "high":
            # SDXL: eval denoise=0.90 → boost timestep 750+
            weights = np.array([weight_mult if t >= 750 else (weight_mid if t >= 500 else 1.0) for t in valid_timesteps])
        elif timestep_weight_bias == "medium":
            # Flux: eval denoise=0.75 → boost timestep 600+
            weights = np.array([weight_mult if t >= 600 else (weight_mid if t >= 400 else 1.0) for t in valid_timesteps])
        else:
            # No bias (original uniform behavior)
            weights = np.ones(len(valid_timesteps))

        weights = weights / weights.sum()
        timesteps = torch.tensor(
            np.random.choice(valid_timesteps, size=b_size, p=weights), device="cpu"
        )
    else:
...
def get_noise_noisy_latents_and_timesteps(
    ...
    # Log timestep bias once at start of training
    if not _timestep_bias_logged:
        bias = os.environ.get("TIMESTEP_WEIGHT_BIAS", "none")
        mult = os.environ.get("TIMESTEP_WEIGHT_MULTIPLIER", "3.0")
        logger.info(f"[Timestep Sampling] bias={bias}, multiplier={mult}")
        _timestep_bias_logged = True
