...
class NetworkTrainer:
    ...
    def get_noise_pred_and_target(
        ...
        return combined_noise_pred, target, timesteps, uncond_noise_pred
    ...
    def process_batch(
        ...
        noise_pred, target, timesteps, uncond_noise_pred = self.get_noise_pred_and_target(
            args,
            accelerator,
            noise_scheduler,
            latents,
        ...
        # Dual loss: weighted combination of CFG-combined loss and unconditional loss
        # This aligns training with eval weighting (75% no_text, 25% text_guided)
        uncond_loss_weight = float(os.environ.get("UNCOND_LOSS_WEIGHT", "0.7"))

        # CFG-combined loss (original)
        loss_combined = train_util.conditional_loss(noise_pred.float(), target.float(), args.loss_type, "none", huber_c)

        if uncond_noise_pred is not None and uncond_loss_weight > 0:
            # Unconditional loss (direct supervision of no_text pathway)
            loss_uncond = train_util.conditional_loss(uncond_noise_pred.float(), target.float(), args.loss_type, "none", huber_c)
            loss = (1.0 - uncond_loss_weight) * loss_combined + uncond_loss_weight * loss_uncond
        else:
            loss = loss_combined
