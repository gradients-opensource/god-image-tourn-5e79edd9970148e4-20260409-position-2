...
"""
konon katanya kita akan sukses 10
"""
...
def run_training(model_type, config_path):
    ...
    # Set timestep weight bias for eval-aligned training
    if model_type == "sdxl":
        os.environ["TIMESTEP_WEIGHT_BIAS"] = "high"      # eval denoise=0.90
    elif model_type == "flux":
        os.environ["TIMESTEP_WEIGHT_BIAS"] = "medium"     # eval denoise=0.75
    else:
        os.environ["TIMESTEP_WEIGHT_BIAS"] = "none"
    os.environ["TIMESTEP_WEIGHT_MULTIPLIER"] = "3.0"      # sweet spot (5.0 was too aggressive)
    os.environ["UNCOND_LOSS_WEIGHT"] = "0.7"               # dual loss: 0.3 combined + 0.7 uncond
    print(f"[Training Config] timestep_bias={os.environ['TIMESTEP_WEIGHT_BIAS']}, "
          f"timestep_mult={os.environ['TIMESTEP_WEIGHT_MULTIPLIER']}, "
          f"uncond_loss_weight={os.environ['UNCOND_LOSS_WEIGHT']}", flush=True)
...
async def main():
    ...
    # Hold-out: disabled — checkpoint selector tidak bisa representasi validator
    # holdout_dir = None
    # train_data_dir = train_paths.get_image_training_images_dir(args.task_id)
    # if args.model_type == "sdxl":
    #     try:
    #         from checkpoint_selector import create_holdout_set
    #         holdout_dir = create_holdout_set(train_data_dir)
    #         if holdout_dir:
    #             print(f"Hold-out set created: {holdout_dir}", flush=True)
    #     except Exception as e:
    #         print(f"⚠️  Hold-out creation failed: {e}. Training tanpa hold-out.", flush=True)
    ...
    # Checkpoint selector: disabled — tidak bisa representasi evaluasi validator
    # if args.model_type == "sdxl":
    #     try:
    #         from checkpoint_selector import select_best_checkpoint
    #
    #         output_dir = train_paths.get_checkpoints_output_path(
    #             args.task_id, args.expected_repo_name
    #         )
    #
    #         print("\n--- Starting Checkpoint Selection ---", flush=True)
    ...
    #         )
    #     except Exception as e:
    #         print(f"⚠️  Checkpoint selection failed: {e}", flush=True)
    #         print("Falling back to last checkpoint (no change)", flush=True)
